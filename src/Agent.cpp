...
float Agent::getAccelerationRate() const
{
...
void Agent::setMaxSpeed(float newSpeed)
{
	m_maxSpeed = newSpeed;
}

void Agent::setAccelerationRate(float rate)
{
...
{
	return m_DetectColour;
}

float Agent::getDetectionRadius()
{
	return m_DetectionRadius;
}

bool Agent::hasDetection()
{
...
void Agent::setHasDetection(bool state)
{
	...
	m_DetectColour = (m_isWithinRadius) ? glm::vec4(0, 1, 0, 1) : glm::vec4(1, 0, 0, 1);
}
